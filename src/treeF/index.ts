 ...
 * @category folds
 */
export const leafF = <A>(value: A): TreeF<A> => ({value})
...
export const branchF: {
  ...
  tupled: <A, C>([value, forest]: [A, Array.NonEmptyArray<C>]) => BranchF<A, C>
} = Object.assign(
  Function.dual(
    2,
...
export const treeF: {
  ...
  2,
  <A, C>(value: A, forest: readonly C[]): TreeF<A, C> =>
    Array.isNonEmptyReadonlyArray(forest) ? {value, forest} : {value},
)
...
export const match =
  ...
  (treeF: TreeF<A, C>): R =>
    isBranch(treeF) ? onBranch(treeF.value, treeF.forest) : onLeaf(treeF.value)
 ...
 * @category folds
 */
export const destruct = <A, C>(self: TreeF<A, C>): readonly [A, C[]] =>
  pipe(
    self,
    match({
      onLeaf: Pair.pair.withSecond([] as C[]),
 ...
 * @category folds
 */
export const getNode = <A, C>({value}: TreeF<A, C>): A => value
...
export const setForest: {
  ...
  <A, B, C>(forest: Array.NonEmptyArray<B>, self: TreeF<A, C>): TreeF<A, B> =>
    pipe(
      self,
      match({
        onLeaf: value => ({value, forest}),
        onBranch: value => ({value, forest}),
      }),
    ),
)
...
export const mapValue: {
  ...
  2,
  <A, B, C>(self: TreeF<A, C>, f: (a: A) => B): TreeF<B, C> =>
    pipe(self, getNode, f, setValue(self)),
)
