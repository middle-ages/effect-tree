...
describe('branch', () => {
  ...
  test('tupled', () => {
    expect(branch.tupled([42, [leaf43]])).toEqual(tree(42, [leaf43]))
  })
})
...
test('destructBranch', () => {
  expect(destructBranch(numericTree as Branch<number>)).toEqual([
    getValue(numericTree),
    getForest(numericTree),
  ])
})
